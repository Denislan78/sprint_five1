...
func (w Walking) Calories() float64 {
	...
	part1 := CaloriesWeightMultiplier * w.Weight
	part2 := (math.Pow(speedMS, 2) / heightM) * CaloriesSpeedHeightMultiplier * w.Weight

	return (part1 + part2) * w.Duration.Hours() * MinInHours
}
...
func (s Swimming) meanSpeed() float64 {
	// вставьте ваш код ниже
	distanceKm := float64(s.Action) * SwimmingLenStep / MInKm
	return distanceKm / s.Duration.Hours()
}
...
func (s Swimming) TrainingInfo() InfoMessage {
	// вставьте ваш код ниже
	distance := float64(s.Action) * SwimmingLenStep / MInKm
	speed := s.meanSpeed()
	calories := s.Calories()
	return InfoMessage{
